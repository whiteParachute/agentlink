...
| AL-TD-002 | Canonical enums and Task / Run / Lease / Device state matrix | Partial | `src/domain/status.ts`; `test/status.test.ts`; in-memory transitions in `src/control-plane/in-memory.ts` | DB-level enum/check usage exists in migration, but no PostgreSQL transition guard/repository yet. |
...
| AL-TD-005 | Run + `al_run_lease` + active lease partial unique index + pull/ack/renew | Partial | `al_run`, `al_run_lease`, `uq_al_run_lease_active`; `agentlet/pull`; `agentlet/ack`; lease tests | `renew` endpoint and PostgreSQL transaction/row-lock implementation are not implemented. |
...
1. Finish AL-TD-002 / AL-TD-005 around a PostgreSQL repository spike: migration runner, DB smoke, transactions, row locks, and active lease conflict tests.
