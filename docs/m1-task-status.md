...
## 2026-06-15 AL-M1-013 Standalone Main Agent outlet update

- Added the AL-M1-013 standalone Main Agent outlet as an explicit routing boundary from resolved `Entry` / `Session` records into the existing Task / Run execution substrate.
- Added `src/domain/main-agent.ts` for stable route idempotency keys (`entry-route:<entryId>`), minimal task payload construction, reply-mode projection embedding, and a fail-closed guard that prevents raw message fields such as `body_text`, `bodyText`, `text`, raw message, transcript, or source payload from entering the Task payload/spec.
- Added control-plane `routeEntryToTask({ entryId })` across in-memory and PostgreSQL-backed implementations. It requires the Entry to already have `sessionId`, validates that the referenced Session exists, reads the optional GroupProfile for AL-M1-009 reply-mode resolution, and then reuses existing `createTask(...)` / `createTaskWithInitialRun(...)` idempotency. No tables, migrations, enums, dependencies, or task-runner behavior were added.
- Added `POST /api/v1/entries/{id}/route-task` behind the existing ingress bearer guard. The endpoint returns `{ task, run, entry, created }` in snake_case DTO form; first route returns 201 and replay returns 200 with the same Task/Run. `/api/v1/meta` now advertises `main-agent-outlet-api`.
- The task payload contains references only: `entry_id`, `session_id`, `source_event_id`, reply-mode resolution, and empty memory/memory-candidate reference arrays. It intentionally does not copy `Entry.bodyText`, raw source payload, full transcript, or context package content.
- Tests cover domain no-raw/idempotency behavior, in-memory first/replay/error paths, PostgreSQL repository create/replay/missing cases via existing Task SQL, HTTP auth/201/200/400/404/route-order/snake_case behavior, and migration invariants proving AL-M1-013 did not add schema.
- Remaining risks: no live PostgreSQL DSN smoke was run unless `AGENTLINK_DATABASE_URL` is provided; true concurrent PostgreSQL idempotency races are covered by the existing Task repository semantics but not a live database. Context package assembly, LLM execution, response gateway, Task runner/agentlet daemon, MemoryBridge, embeddings/search, real Feishu/OAuth/webhook productization, frontend expansion, multi-tenant/multi-MainUser, and complex routing policy remain intentionally out of scope.

## 2026-06-15 AL-M1-014 Minimal agentlet daemon loop + fake runner consume update

- Added the AL-M1-014 minimal, test-drivable consume step that completes one memory-first vertical: a routed `Task`/`Run` is now consumed to a terminal, observable run through the existing agentlet `pull` / `ackLease` / `appendProgress` / `completeRun` surface plus a `RunnerAdapter`. It is intentionally not a long-running production daemon and does not perform real Codex CLI execution.
- Added `src/agentlet/daemon.ts` exporting `daemonStep(deps, input)` and its outcome types. The step pulls one queued run (returns `{ kind: 'idle' }` when none), acknowledges the lease, builds the runner input via `runnerInputFromInstruction(...)`, runs the injected `RunnerAdapter`, replays each `RunnerEvent` through `runnerEventToAgentletProgress(...)` into `appendProgress(...)`, and finalizes via `completeRun(...)`. A runner that returns `FAILED`/`CANCELLED` or throws drives the run to the matching terminal state instead of leaving the lease dangling; a `completeRun` failure is surfaced to the caller.
- Added `src/agentlet/fake-runner.ts` (`FakeRunnerAdapter`): a deterministic, dependency-free adapter that emits scripted events and a terminal result without spawning any process. It is suitable for tests and dry-runs and never executes Codex or any command.
- Reused the existing `AgentlinkControlPlanePort` and `RunnerAdapter` contracts only. No migrations, schema, enum, package/lock, dependency, frontend, control-plane port, or HTTP route changes were added; the loop is driven by callers/tests invoking `daemonStep` repeatedly.
- Tests (`test/agentlet-daemon.test.ts`) cover no-work idle, success consume with observable run events, runner `FAILED` result, a thrown runner mapped to terminal FAILED, and a full memory-first vertical (fake-im ingress → `resolveSession` → `routeEntryToTask` → `daemonStep` terminal complete) including a no-raw assertion that the routed Task payload does not leak `Entry.bodyText`.
- Remaining risks: no live PostgreSQL DSN smoke was run unless `AGENTLINK_DATABASE_URL` is provided; the vertical is exercised against the in-memory control plane only. Production resident daemon/loop scheduling, multi-device fallback, real Codex CLI E2E, response gateway, context package assembly, MemoryBridge, LLM, embeddings/search, real Feishu/TG/OAuth/webhook productization, frontend expansion, and multi-tenant/multi-MainUser remain intentionally out of scope.
