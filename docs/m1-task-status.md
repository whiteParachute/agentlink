...
| AL-TD-006 | `control_actions` / poll + recover + cancel | Partial | `POST /api/v1/tasks/:taskId/cancel`; `POST /api/v1/agentlet/control/poll`; `POST /api/v1/agentlet/control/ack`; `POST /api/v1/agentlet/recover`; `POST /api/v1/agentlet/recover/decision`; `al_control_action`; in-memory control actions; PostgreSQL `cancelTask`, `listControlActionsForDevice`, `ackControlAction`, `listRecoverableRunsForDevice`, `recoverContinue`, `recoverDiscard`; HTTP and repository tests | Minimal M1 control protocol now covers cancel, poll, ack retention, renew envelope, recover continue, and recover discard. Remaining gaps: device revoke cascade API, real agentlet consumption, live DSN validation, and long-term control-action retention cleanup policy. |
...
2. Add AL-TD-003 grant management/revoke APIs and decide whether device revoke cascade belongs before the Codex runner slice.
